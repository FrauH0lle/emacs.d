...
**How to apply:** When adding more cases for an already-tested symbol, extend the existing `zenit-deftest` form instead of creating another `zenit-deftest` with a slash suffix. For multiple independently documented cases, use the template shape with `,test` and `(test)`, then per-case `:doc` entries; do not conclude multiple `:doc` cases are unsupported just because they fail in the simple-test form.
