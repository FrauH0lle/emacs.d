...
(defmacro zenit-autoloads--wrap-print (&rest body)
  (macroexp-progn
...
(defun zenit-autoloads--write (file header &rest forms)
  (unless (eq (car forms) 'skip)
    (make-directory (file-name-directory file) 'parents)
    (condition-case-unless-debug e
        (with-temp-file file
          (zenit-autoloads--wrap-print
           (when header
...
(defun zenit-autoloads--scan (file &optional literal source)
  (let (autoloads)
    (when (file-readable-p file)
      (setq source (file-truename (or source file)))
      (with-temp-buffer
        (insert-file-contents file)
        (save-excursion
...
(defun zenit--generate-autoloads ()
  (let ((target-file (file-name-concat zenit-local-dir "zenit-autoloads.el"))
        (temp-file (file-name-concat zenit-local-dir "temp-zenit-autoloads.el"))
        (files (append (zenit-glob (file-name-concat zenit-core-dir "lib") "*.el")
                       (cl-loop for dir
    ...
    (with-temp-file target-file
      (cl-loop for file in files
               ...
               for module = (zenit-module-from-path file)
               for module-enabled-p = (and (zenit-module-p (car module) (cdr module))
                                           (zenit-file-cookie-p file "if" t))
               ;; Collect autodefs
               for autodefs = (let (forms)
                                (with-temp-buffer
                                  (zenit-autoloads--scan-autodefs file (current-buffer) module module-enabled-p)
                                  (goto-char (point-min))
                                  (condition-case _
                                      (while t
               ...
               (let (;; Prevent `autoload-find-file' from firing file hooks, e.g. adding
                     ...
                     (inhibit-message t))
                 (loaddefs-generate
                  (file-name-directory file) temp-file (remove file (zenit-glob (file-name-directory file) "*.el")) nil nil t))
               if autodefs
               do
               (with-temp-buffer
                 (zenit-autoloads--wrap-print
                  (mapc #'prin1 (delq nil autodefs)))
                 (write-region (point-min) (point-max) temp-file t))
               do
               (zenit-autoloads--wrap-print
                (mapc #'prin1 (zenit-autoloads--scan temp-file nil file)))))

    (delete-file temp-file))
  'skip)

(defun zenit--generate-package-autoloads ()
  (let ((target-file (file-name-concat zenit-local-dir "zenit-packages-autoloads.el")))
    ;; Ensure file can be created
    (make-directory (file-name-directory target-file) 'parents)

    (with-temp-file target-file
      (zenit-autoloads--wrap-print
      (dolist (file (mapcar #'straight--autoloads-file
                          (nreverse (seq-difference (hash-table-keys straight--build-cache)
                                                    zenit-autoloads-excluded-packages))))
      (when (file-exists-p file)
        (zenit-log "Scanning %s" file)
        (mapc #'prin1 (zenit-autoloads--scan file 'literal)))))))
  'skip)
