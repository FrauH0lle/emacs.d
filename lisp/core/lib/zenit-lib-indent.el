;; lisp/core/lib/zenit-lib-indent.el -*- lexical-binding: t; -*-
...
;;;###autoload
(progn
  (defvar zenit-indent-excluded-modes '(special-mode org-mode)
    "Modes not to no-op `zenit-set-indent' in.")

  (defun set-indent-vars! (modes vars)
    "Register VARS as the tab-width proxy variables for each of major MODES.

`tab-width' and `standard-indent' will be set to match the values of
these variables in their respective buffers. Failing that, a best-guess
effort will be made to find an appropriate variable to use.

If MODES is `t', VARS will be made to match `tab-width' in all major
modes. Both MODES and VARS can be a single symbol or a list thereof.
...
MODE-tab-width, or MODE-basic-offset. It is also not necessary if you
have `dtrt-indent' installed and a variable association exists in
`dtrt-indent-hook-mapping-list'."
  ...
  (defun zenit-indent-var-for-mode (mode)
...
with `set-indent-vars!'."
    (unless (provided-mode-derived-p mode zenit-indent-excluded-modes)
  ...
  (defun zenit-indent-vars-for-mode (mode)
    ...
    (unless (eq mode 'fundamental-mode)
      (or (get mode 'indent-vars)
          ...
          (ensure-list (zenit-indent-var-for-mode mode)))))

  (defun zenit-set-indent (&optional width)
    "Ensure tab-width reflects the local major mode's indent variable."
    (when-let* ((vars (zenit-indent-vars-for-mode major-mode))
                (width* (or width
                            (cl-loop for v in vars
                                     if (boundp v)
                                     if (symbol-value v)
                                     return it))))
      (zenit-log 2 "zenit-set-indent: %S = %S" `(tab-width standard-indent ,@vars) width*)
