...
(defconst zenit-core-dir (file-name-directory load-file-name)
  "The root directory of the config's core files.
...
Use this for: server binaries, pulled module libraries, generated files,
...
Use this for: images/data caches, elisp bytecode, natively compiled
elisp, session files, ELPA archives, authinfo files, org-persist, etc.

Must end with a slash.")

(defvar zenit-state-dir (file-name-concat zenit-local-dir "state/")
  "Directory for global state files.

Use this for: history, logs, user-saved data, autosaves/backup files,
known projects, recent files, bookmarks, etc.

Must end with a slash.")

(defvar zenit-bootstrap-dir (file-name-concat zenit-data-dir "zenit-bootstrap-files/")
  "Directory where autogenerated init files are stored.

Use this for file generated for CLI operations like building package
autoloads file. Must end with a slash.")
...
environment, which is loaded at startup (if it exists). This is helpful
if Emacs can't (easily) be launched from the correct shell
session (particularly for MacOS users).")

(defconst zenit-config-init-file
  (file-name-concat user-emacs-directory "init.el")
  "Zenit Emacs main init file.

This file contains all information to load up the user session.")
...
Init files are loaded early, just after the core, and before modules'
config files. They are always loaded, even in non-interactive sessions,
and before `zenit-before-modules-init-hook'. Related to
...
sessions. These run before `zenit-after-modules-config-hook' and after
...
Control files define dependencies and conflicts, along other metadata.
They are rarely read in interactive sessions.")
...
;; Don't litter
(setq async-byte-compile-log-file  (concat zenit-state-dir "async-bytecomp.log")
