...
(use-package! corfu
  ...
  :hook
  (zenit-first-buffer . global-corfu-mode)
  :config
  ...
  (add-hook 'zenit-init-modules-hook
            (lambda ()
              (after! lsp-mode
                (setq lsp-completion-provider :none))))

  ;; Set orderless filtering for LSP-mode completions
  ;; TODO: expose a variable to control this part
...
(use-package! kind-icon
  ...
  (setq kind-icon-use-icons t
        svg-lib-icons-dir (expand-file-name "svg-lib" zenit-cache-dir)
        kind-icon-mapping
  ...
  (add-hook 'zenit-load-theme-hook #'kind-icon-reset-cache)
...
(when (modulep! :editor evil)
  (after! (evil corfu)
    ...
    (+evil-collection-init 'corfu +evil-collection-disabled-list)))
