...
With non-nil prefix INCLUDE-ROOT, also include the project's root."
  ...
  (+default/yank-buffer-path
   (if include-root
       (file-name-directory (directory-file-name (zenit-project-root)))
     (zenit-project-root))))
...
;;;###autoload
(defun zenit/backward-delete-whitespace-to-column ()
...
+ Otherwise, resort to `zenit/backward-delete-whitespace-to-column'.
         ...
         (cond ((or (modulep! -smartparens)
                    ...
                    (and (memq (char-before) (list ?\  ?\t))
                         (save-excursion
                           (and (/= (skip-chars-backward " \t" (line-beginning-position)) 0)
                                (bolp)))))
                (zenit/backward-delete-whitespace-to-column))
               ((let* ((pair (ignore-errors (sp-get-thing)))
                  ...
                  (cond ((and end beg (= end (+ beg (length op) (length cl))))
                        ...
                        ((zenit-surrounded-p pair 'inline 'balanced)
                         (delete-char -1 killflag)
                         (delete-char 1)
                         (when (= (point) (+ (length cl) beg))
                           (sp-backward-delete-char 1)
                           (sp-insert-pair op)))
                        ((and (bolp) (zenit-surrounded-p pair nil 'balanced))
                         (delete-region beg end)
                         (sp-insert-pair op)
                         t)
                        ((run-hook-with-args-until-success 'zenit-delete-backward-functions))
                        ((zenit/backward-delete-whitespace-to-column)))))))
