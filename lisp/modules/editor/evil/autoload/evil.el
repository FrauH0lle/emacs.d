...
By default, this replaces the selection with what's in the
clipboard without replacing its contents."
  (interactive)
...
If there are no windows there and there is only one window, split in
that direction and place this window there. If there are no windows and
this isn't the only window, uses evil-window-move-* (e.g.
`evil-window-move-far-left').

If already at the edge of the frame and
`+evil-want-move-window-to-wrap-around' is non-nil, move the window to
the other end of the frame. Inverts
...
If `evil-split-window-below' is non-nil, the new window isn't
focused."
  (interactive)
...
If `evil-vsplit-window-right' is non-nil, the new window isn't
focused."
  (interactive)
...
(evil-define-operator +evil:retab (&optional beg end)
  "Wrapper around `zenit/retab'."
  :motion nil :move-point nil :type line
  (interactive "<r>")
  (zenit/retab nil beg end))
...
Widens narrowed buffers first. If BANG, use indirect buffer
clones instead."
  :move-point nil
  (interactive "<r><!>")
  (if (not bang)
    ...
    (when (buffer-narrowed-p)
      (zenit/widen-indirectly-narrowed-buffer t))
    (zenit/narrow-buffer-indirectly beg end)))
...
(evil-define-operator +evil:yank-unindented (beg end _type _register _yank-handler)
  "Saves the (reindented) characters in motion into the
kill-ring."
  :move-point nil
  :repeat nil
  (interactive "<R><x><y>")
...
(evil-define-operator +evil-delete (beg end type register yank-handler)
  "A wrapper around `evil-delete' for `wgrep' buffers that will
invoke `wgrep-mark-deletion' on lines you try to delete."
  (interactive "<R><x><y>")
  (condition-case _ex
