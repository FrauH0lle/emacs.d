...
PATTERN is a vim-style regexp. FLAGS is an optional string of
...
If BANG, then run ARGUMENTS as a full command. This command
understands vim file modifiers (like %:p:h). See
...
(evil-define-command +evil:kill-all-buffers (&optional bang)
  ...
  (if (and bang (fboundp '+workspace/kill-session))
    ...
    (call-interactively #'zenit/kill-all-buffers)))
...
(evil-define-command +evil:kill-matching-buffers (&optional bang pattern)
  "Kill all buffers matching PATTERN regexp. If BANG, only match
project buffers."
  (interactive "<a>")
