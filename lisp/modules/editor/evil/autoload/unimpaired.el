...
(defun +evil/next-comment (count)
  ...
  (let ((orig-pt (point)))
    ...
    (dotimes (_ (abs count))
      (cond ((> count 0)
             (while (and (not (eobp)) (zenit-point-in-comment-p))
               (forward-line 1))
             (unless (comment-search-forward (point-max) 'noerror)
               (goto-char orig-pt)
               (user-error "No comment after point")))
            (t
             (while (and (not (bobp)) (zenit-point-in-comment-p))
               (forward-line -1))
                        ...
                        (let ((filename (file-name-nondirectory file)))
                          (save-match-data
                            (not (and (bound-and-true-p flycheck-mode)
                                      ...
                                      ;; Make extra sure it's a generated file
                                      ;; and not the actual flycheck library!
                                      (file-exists-p (file-name-concat (file-name-directory file) (match-string 1)))))))))
                 (zenit-glob (file-name-directory buffer-file-name) "[!.]*")))
