...
;; Only take the parts we need/want from evil-collection. Here, we set up the
;; loader and internal/core packages. The rest should be in the corresponding
...
Unlike `evil-collection-init', this respects
`+evil-collection-disabled-list', and complains if a module is
loaded too early (during startup)."
    (unless (memq (or (car-safe module) module) disabled-list)
      (zenit-log "editor:evil: loading evil-collection-%s %s"
  ...
  (after! evil
    ...
    ;;      makes it hard to overwrite these keybinds with a simple `after!' or
