...
1. Enable partial/region reformatting, while preserving leading
   indentation,
2. Applies changes via RCS patch, line by line, to protect buffer
   markers and reduce cursor movement or window scrolling.

See `+format/buffer' for the interactive version of this function."
  (cond
   ((eq formatter 'lsp)
    (call-interactively
      ...
      (pcase-let*
        ...
        (unwind-protect
            (when (eq status :reformatted)
              (let ((tmpfile (make-temp-file "zenit-format"))
                    (patchbuf (get-buffer-create " *zenit format patch*"))
