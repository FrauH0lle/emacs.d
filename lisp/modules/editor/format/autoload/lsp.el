...
(define-minor-mode +format-with-lsp-mode
  "Toggles `lsp-mode'/`eglot' integration with `apheleia-mode' in
 this buffer.

When enabled, it changes `+format-with' to `lsp', and back to its
old value when disabled. Use `+format-wtih-lsp-maybe-h' to
activate it, to ensure it only activates if lsp-mode/eglot are on
and connected to a valid client, and so this mode won't conflict
with pre-existing user config on `+format-with'."
  :init-value nil
...
(defvar +format--lsp-alist
  ...
  "A list of LSP formatter functions to try on
 `+format-lsp-buffer'.")
...
(defun +format-with-lsp-toggle-h ()
  "Toggle `+format-with-lsp-mode' depending on the state of
 lsp-mode/eglot.
...
(cl-defun +format-lsp-buffer (&rest plist &key buffer callback &allow-other-keys)
  "Format the current buffer with any available lsp-mode or eglot
 formatter."
...
(cl-defun +format--with-lsp-mode (&key buffer scratch &allow-other-keys)
  "Format the current buffer with any available lsp-mode
 formatter."
