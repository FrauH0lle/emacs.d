...
(defvar +word-wrap-extra-indent 0
...
When \\='auto, long lines will soft-wrap at `fill-column'. If
`auto-fill-mode' is enabled, its behaviour will not be affected.

When \\='soft, long lines will soft-wrap at `fill-column' and
`auto-fill-mode' will be forcibly disabled.

Otherwise long lines will soft-wrap at the window margin and
`auto-fill-mode' will not be affected.")
...
(defvar +word-wrap-visual-modes
  '(org-indent-mode markdown-indent-mode)
...
;; REVIEW: `visual-fill-column' isn't maintained, so it likely won't happen
;;   upstream.
