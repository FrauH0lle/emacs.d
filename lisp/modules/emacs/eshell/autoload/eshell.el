...
(defvar eshell-buffer-name "*zenit:eshell*")
...
(defun +eshell-lookup-documentation (cmd)
  ...
  (cond ((eshell-find-alias-function cmd)
        ...
        ((and (or (and (string-match-p "^\\*." cmd)
              ...
              (zerop (car (zenit-call-process manual-program cmd))))
...
(defun +eshell/switch-to (buffer)
  ...
  (interactive
   (let ((buffers (zenit-buffers-in-mode
        ...
        (cond ((and (one-window-p t)
              ...
              ((one-window-p)
               (let ((prev (save-window-excursion (previous-buffer))))
                 (unless (and prev (zenit-real-buffer-p prev))
                   (switch-to-buffer (zenit-fallback-buffer)))))
