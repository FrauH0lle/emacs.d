...
(defun +org-init-appearance-h ()
  ...
  (setq org-refile-targets
        ...
        ;; Without this, completers like ivy/helm are only given the first level
        ;; of each outline candidates. i.e. all the candidates under the "Tasks"
        ;; heading are just "Tasks/". This is unhelpful. We want the full path
        ;; to each refile target! e.g. FILE/Tasks/heading/subheading
  ...
  (defadvice! +org-inhibit-mode-hooks-a (fn datum name &optional initialize &rest args)
    ...
    (apply fn datum name
           (if (and (eq org-src-window-setup 'switch-invisibly)
               ...
               ;; major mode with expensive hooks can be terribly expensive. As
               ;; we add the most expensive hooks to MAJOR-MODE-local-vars-hook,
               ;; we can savely inhibit those.
               (lambda ()
...
(defun +org-init-custom-links-h ()
  ...
  (pushnew! org-link-abbrev-alist
            ...
            '("zenit-emacs-repo" . "https://github.com/FrauH0lle/emacs.d/%s")
            `("emacsdir"         . ,(zenit-path zenit-emacs-dir "%s"))
            `("localconfir"      . ,(zenit-path zenit-local-conf-dir "%s")))
...
an indirect buffer."
    ...
    (defadvice! +org--optimize-backgrounded-agenda-buffers-a (fn file)
      "Disable `org-mode's startup processes for temporary agenda
buffers.

Prevents recentf pollution as well. However, if the user tries to
visit one of these buffers they'll see a gimped, half-broken org
buffer, so to avoid that, install a hook to restart `org-mode'
