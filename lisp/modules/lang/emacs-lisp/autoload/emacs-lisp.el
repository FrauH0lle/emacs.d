...
Meant as an eval handler for the :tools eval module."
...
(defun +emacs-lisp--module-at-point ()
  "Return (CATEGORY MODULE FLAG) at point inside a `modules!' block."
  (let ((origin (point))
        (syntax (syntax-ppss)))
      ...
      (save-excursion
        (let ((parens (ppss-open-parens syntax))
          ...
          (while (and parens (progn (goto-char (car parens))
                                    (not (looking-at "(modules!\\_>"))))
            (setq parens (cdr parens)
                  zenit-depth (1+ zenit-depth)))
          (when parens ;; Are we inside a `modules!' block?
            (goto-char origin)
            (let* ((zenit-start (car parens))
                   (bare-symbol
                    (if (ppss-comment-depth syntax)
                        (= (save-excursion (beginning-of-thing 'list)) zenit-start)
                      (null (cdr parens))))
...
If ALL-BUFFERS (the prefix arg), the prompted buffer list will include
all buffers (if don't have the workspaces module enabled, there's no
difference).

This only affects the buffer-local value of
`+emacs-lisp-working-buffer'. If BUFFER is nil or the focused buffer,
unsets `+emacs-lisp-working-buffer's local value."
  (interactive
   (list (read-buffer
          "Set working buffer to: " (list (current-buffer))
          nil (unless all-buffers
                (let ((buffers (mapcar #'buffer-name (zenit-buffer-list))))
                  (lambda (b)
...
If the repl isn't already open, sets ielm's working buffer to the
buffer selected before this command was invoked."
  (interactive)
  (pop-to-buffer
   (or (get-buffer "*ielm*")
...
(defun +emacs-lisp/buttercup-run-file ()
  ...
  (interactive)
  (let ((load-path
         (append (list (zenit-path (dir!) "..")
                       (or (zenit-project-root)
                           default-directory))
                 load-path))
...
(defun +emacs-lisp/buttercup-run-project ()
  ...
  (interactive)
  (let* ((default-directory (zenit-project-root))
         (load-path (append (list (zenit-path "test")
                                  default-directory)
                            load-path))
         (buttercup-suites nil))
...
(defun +emacs-lisp-init-straight-maybe-h ()
  ...
  (when (file-in-directory-p (or buffer-file-name default-directory) zenit-local-dir)
                 ...
                 (when (re-search-backward "^ *\\((provide\\)\\(?:-theme\\)? +'"
                   ...
                   (ignore-errors
                     (and (stringp file-base)
                          (equal (symbol-name (zenit-unquote (nth 1 (read (current-buffer)))))
                                 file-base))))))))))
    ...
    (setq +emacs-lisp-reduced-flymake-byte-compile--process
          (make-process
           ...
           :command `(,(expand-file-name invocation-name invocation-directory)
                      ...
                      "--eval" ,(prin1-to-string `(setq zenit-modules ',zenit-modules
                                                        zenit-disabled-packages ',zenit-disabled-packages
