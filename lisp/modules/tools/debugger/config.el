...
(defvar +debugger--dap-alist
  ...
  "An alist mapping modules to major mode and DAP packages.")
...
(use-package! dape
  ...
  (defadvice! +debugger--make-directory-a (&optional file)
    ...
    (unless (file-exists-p (or file dape-default-breakpoints-file))
      (make-empty-file (or file dape-default-breakpoints-file) t))))
...
(use-package! dap-mode
  ...
  :init
  (setq dap-breakpoints-file (concat zenit-data-dir "dap-breakpoints")
        dap-utils-extension-path (concat zenit-data-dir "dap-extension/"))
  (after! lsp-mode (require 'dap-mode))
  :config
  (pcase-dolist (`((,category . ,modules) :after ,after :require ,libs)
                 +debugger--dap-alist)
    (when (zenit-module-p category (car modules) (cadr modules))
