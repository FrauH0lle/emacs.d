...
Evaluate as in RUNNER-MAJOR-MODE. If RUNNER-MAJOR-MODE is nil,
use major-mode of the buffer instead."
...
(defun +eval/buffer-or-region ()
  "Evaluate the region if it's active, otherwise evaluate the whole
buffer.

If a REPL is open the code will be evaluated in it, otherwise a
quickrun runner will be used."
  (interactive)
  (call-interactively
   (if (use-region-p)
       #'+eval/region
...
(defun +eval/region-and-replace (beg end)
  "Evaluation a region between BEG and END, and replace it with the
result."
  (interactive "r")
  (let (lang)
    (cond
