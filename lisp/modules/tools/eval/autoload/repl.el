...
(defun +eval-open-repl (prompt-p &optional displayfn)
  "Open a repl via the given DISPLAYFN. If PROMPT-P, the user will
be prompted for a repl choice, even if the major mode they're in
...
Opens a REPL if one isn't already open. If AUTO-EXECUTE-P, then
            ...
            (if inhibit-auto-execute-p
              ...
              (if (bound-and-true-p evil-local-mode)
                  (let (evil-move-cursor-back)
                    (evil-save-state
                      (evil-append-line 1)
                      (call-interactively (zenit-lookup-key (kbd "RET")))))
                (call-interactively (zenit-lookup-key (kbd "RET")))))))))))
