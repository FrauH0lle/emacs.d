...
DOCSETS can be strings, each representing a dash docset, or a
vector with the structure [DOCSET FORM]. If FORM evaluates to
nil, the DOCSET is omitted. If it is non-nil, (format DOCSET
FORM) is used as the docset.

The first element in DOCSETS can be :add or :remove, making it
easy for users to add to or remove default docsets from modes.
...
(defun +lookup-dash-docsets-backend-fn (identifier)
  "Looks up IDENTIFIER in available Dash docsets, if any are
installed.
...
QUERY is a string and docsets in an array of strings, each a name
of a Dash docset.

If prefix ARG is supplied, search all installed installed
docsets. They can be installed with `dash-docs-install-docset'."
  (interactive "P")
  (require 'dash-docs)
  (let ((dash-docs-common-docsets)
        ...
        (query (zenit-thing-at-point-or-region query)))
    (zenit-log "Searching docsets %s" dash-docs-docsets)
