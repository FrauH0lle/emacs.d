...
;;
;; This module uses `xref' and will fall back to `dumb-jump' to find what you
;; want.
...
(defvar +lookup-provider-url-alist
  ...
  1. A search url (needs on '%s' to substitute with an url
     encoded query),

  2. A non-interactive function that returns the search url in
     #1,
...
(defvar +lookup-definition-functions
  ...
  "Functions for `+lookup/definition' to try, before resorting to
`dumb-jump'. Stops at the first function to return non-nil or
...
(defvar +lookup-implementations-functions ()
  "Function for `+lookup/implementations' to try. Stops at the first
function to return non-nil or change the current window/point.
...
(defvar +lookup-references-functions
  '(+lookup-thesaurus-definition-backend-fn
    ...
    +lookup-project-search-backend-fn)
  "Functions for `+lookup/references' to try, before resorting to
`dumb-jump'. Stops at the first function to return non-nil or
change the current window/point.
...
(defvar +lookup-file-functions
  ...
  "Function for `+lookup/file' to try, before restoring to
`find-file-at-point'. Stops at the first function to return
non-nil or change the current window/point.
...
Setting this to nil will force it to use offline backends, which
may be less than perfect, but available without an internet
connection.

Used by `+lookup/dictionary-definition' and `+lookup/synonyms'.

For `+lookup/dictionary-definition', this is ignored on Mac,
where Emacs users Dictionary.app behind the scenes to get
definitions.")
...
(use-package! dumb-jump
  ...
  :config
  (setq dumb-jump-default-project zenit-emacs-dir
...
(use-package! dash-docs
  ...
  (setq dash-docs-enable-debugging init-file-debug
        dash-docs-docsets-path (concat zenit-data-dir "docsets/")
...
;;;###package synosaurus
(setq synosaurus-choose-method 'default) ; use minibuffer
