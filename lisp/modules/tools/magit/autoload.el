...
- Magit process windows are always opened in small windows below
  the current.
...
This differs from `display-buffer-in-direction' in one way: it
will try to use a window that already exists in that direction.
