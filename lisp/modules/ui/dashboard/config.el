...
(defcustom +dashboard-name "*dashboard*"
...
Possible values:
  \\='last-project
    The `zenit-project-root' of the last open buffer. Falls back to
...
(defvar +dashboard-inhibit-functions ()
  "A list of functions which take no arguments.
If any of them return non-nil, dashboard reloading is inhibited.")
...
(defface +dashboard-banner '((t (:inherit font-lock-comment-face)))
  "Face used for the banner on the dashboard"
  :group '+dashboard)
...
(add-hook 'zenit-init-ui-hook #'+dashboard-init-h 'append)

;; Make sure the dashboard is ready early, so as to avoid triggering
;; `zenit-first-buffer-hook' later, when switching to it.
(when (equal (buffer-name) "*scratch*")
  (let (buffer-list-update-hook
        zenit-first-buffer-hook)
...
if the buffer is real. See `zenit-real-buffer-p' for an explanation for what
\\='real' means.
...
If this is the dashboard buffer, reload it completely."
  (cond ((+dashboard-buffer-p (current-buffer))
        ...
        ((and (not (file-remote-p default-directory))
              (zenit-real-buffer-p (current-buffer)))
...
integration with the dashboard. It ensures that the dashboard is always
in the correct project (which may be different across perspective)."
...
What it is set to is controlled by `+dashboard-pwd-policy'."
  (if pwd
      (with-current-buffer (zenit-fallback-buffer)
        (zenit-log "Changed dashboard's PWD to %s" pwd)
...
(defun +dashboard-reload (&optional force)
  "Update the scratch buffer (or create it, if it doesn't exist)."
  (when (or (and (not +dashboard-inhibit-refresh)
                 (get-buffer-window (zenit-fallback-buffer))
                 (not (window-minibuffer-p (frame-selected-window)))
                 (not (run-hook-with-args-until-success '+dashboard-inhibit-functions)))
            force)
    (with-current-buffer (zenit-fallback-buffer)
      (zenit-log "Reloading dashboard at %s" (format-time-string "%T"))
      (with-silent-modifications
        (let ((pt (point)))
