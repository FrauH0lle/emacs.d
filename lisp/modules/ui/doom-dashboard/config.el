...
(defvar +doom-dashboard-banner-file "default.png"
  "The path to the image file to be used in on the dashboard. The
...
(defvar +doom-dashboard-ascii-banner-fn #'doom-dashboard-draw-ascii-banner-fn
  "The function used to generate the ASCII banner on Doom's
dashboard.")
...
(defvar +doom-dashboard-pwd-policy 'last-project
  "The policy to use when setting the `default-directory' in the
dashboard.
...
and before the key string.

PLIST can have the following properties:

  :icon FORM
    Uses the return value of FORM as an icon (can be literal
    string).
  :key STRING
...
(defvar +doom-dashboard--width 80
  "Variable to store the width of the dashboard buffer.")
(defvar +doom-dashboard--old-fringe-indicator fringe-indicator-alist
"Variable to store the original fringe indicator.")
(defvar +doom-dashboard--pwd-alist ()
  "Variable to store the current working directory alist.")
...
(defun +doom-dashboard-init-h ()
  ...
  (unless noninteractive
    ...
    ;; else to show.
    (setq zenit-fallback-buffer-name +doom-dashboard-name
    ...
    (when (equal (buffer-name) "*scratch*")
      (set-window-buffer nil (zenit-fallback-buffer))
      (+doom-dashboard-reload))
    (add-hook 'zenit-load-theme-hook #'+doom-dashboard-reload-on-theme-change-h)
    ...
    (insert
     ...
     (propertize
      (+doom-dashboard--center
       +doom-dashboard--width
       (zenit-display-benchmark-h 'return))
